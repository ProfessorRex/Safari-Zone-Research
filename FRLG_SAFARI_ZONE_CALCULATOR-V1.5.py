...
def one_bait_catch(catch_rate, flee_rate):
    ...
    p_failure = p_flee
    p_success = 0
    # Throwing the bait will grant between 1-5 turns of "eating"
    for i in range(1, 6):
...
def bait_n_ball_catch(catch_rate, flee_rate):
    ...
    while round_vals[0] > 0.000001:
        #run sims for each time bait wears off
...
def pretty_outputs(catch_rate, flee_rate, name):
    ...
    print("Odds of capture with a pattern of (Bait, Ball) ->")
    print("(Bait, Ball, Ball, Ball, Bait, Ball, Ball) Repeating: " + str(odds[0] * 100)[:7] + "%")


def pattern_odds_catch_easy(catch_rate, flee_rate, turns, p_turn=1,
                            amount_of_bait=0, baited=False, big_turns=3, t=0):
    '''
    ...
    if len(turns) > 0:
        ...
        p_success = p_turn * big_turns
        p_failure = p_turn - p_success
        return (p_success, p_failure)
    if big_turns == 3:
        #plug turns list into t
        t = turns
        # Get a Low Estimate success rate for the extra balls not in pattern
        # Get number of balls
        balls = turns.count('L')
        new_turns = ''
        i = 0
        while balls < 30:
            new_turns = new_turns + turns[i]
            if turns[i] == 'L':
                balls +=1
            i += 1
            if i + 1 > len(turns):
                i = 0
        big_turns = pattern_odds_catch_easy(catch_rate, flee_rate, new_turns,
                                            1, 0, True, 0.18, t)[0]
