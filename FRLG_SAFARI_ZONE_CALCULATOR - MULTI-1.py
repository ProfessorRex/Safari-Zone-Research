        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            ...
            if amount_of_bait <= 0:
                for i in (2, 3, 4, 5, 6):
                    ...
                    future = executor.submit(pattern_odds_catch_deep, p_flee_watching, p_flee_eating, p_catch_baited, p_catch_unbaited, turns[1:], r, p_add_curr_bait, new_bait, True, t)
                    futures.append(future)
                for i in (0, 1, 2, 3, 4):
                    round_vals = futures[i].result()
                    p_success = p_success + round_vals[0]
                    p_failure = p_failure + round_vals[1]
